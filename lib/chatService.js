import { portfolioKnowledge, buildKnowledgePrompt } from '@/lib/portfolioKnowledge'
...
/**
 * Hybrid chatbot:
 * 1) Groq (or OpenAI) when API key is set
 * 2) Rule-based localAnswer fallback when no key / LLM fails
 */
async function answerChat({ message, history }) {
  try {
    ...
    console.error(`[chat] LLM fallback → local rules: ${error.message}`);
  }

  return { reply: localAnswer(message), mode: 'local' };
}

export { answerChat, localAnswer }
